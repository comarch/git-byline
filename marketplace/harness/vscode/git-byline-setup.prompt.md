...
Copy `marketplace/harness/vscode/hooks.json` from the git-byline repository into
`.github/hooks/promptscript-vscode.json` of the user's project, creating parent directories as needed:

```sh
curl -fsSL --proto '=https' --proto-redir '=https' --tlsv1.2 \
  -o .github/hooks/promptscript-vscode.json --create-dirs \
  https://raw.githubusercontent.com/comarch/git-byline/main/marketplace/harness/vscode/hooks.json
```

The same file is checked in beside this command, so it can also be copied
from a local clone. It calls `git-byline checkpoint portable-vscode
