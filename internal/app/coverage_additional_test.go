...
func TestCoverageEnvironmentFallbacks(t *testing.T) {
	...
	_, deepErr := (&Env{}).workingDir()
	if _, outputErr := resolveOutputPath(&Env{}, "report.json", "test"); outputErr == nil {
		t.Fatal("resolveOutputPath succeeded in an invalid current directory")
	}
	for _, args := range [][]string{
		...
		if code, runErr := Run(args, env); code != ExitFailure || runErr == nil {
			t.Fatalf("Run(%v) in deep cwd = %d, %v", args, code, runErr)
		}
	}
	...
	if deepErr == nil {
		t.Log("workingDir resolved a deeply nested current directory")
	}
	...
	for _, args := range [][]string{
		...
		if code, runErr := Run(args, env); code != ExitFailure || runErr == nil {
			t.Fatalf("Run(%v) in unreadable cwd = %d, %v", args, code, runErr)
		}
	}
	restoreErr = os.Chmod(parent, 0o700)
	if restoreErr == nil {
		restoreErr = os.Chdir(original)
	}
	if restoreErr != nil {
	...
	if err == nil {
		t.Log("workingDir resolved a directory with an unreadable parent")
	}
}
