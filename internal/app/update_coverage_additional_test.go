...
func TestRunUpdateDryRunAndFlagErrors(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		options, _ = stageZipUpdate(t, "git-byline_1.0.0_windows_amd64.zip",
			releaseZipEntries("new binary"))
	} else {
...
func TestCurrentExecutableSymlinkFailure(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("Windows cannot remove the running test executable")
	}
...
func TestUpdateTarReadFailures(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		t.Skip("read-only output descriptor failure is not portable on Windows")
	}
...
func TestApplyUpdateSwapFailure(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("directory target does not trigger a swap failure on Windows")
	}
	t.Parallel()
	options, _ := stageTarUpdate(t, "git-byline_1.0.0_linux_amd64.tar.gz",
		releaseTarEntries("new binary"))
