...
func TestCoveragePendingRewriteBranches(t *testing.T) {
	...
	t.Run("rewritten note encode error", coverageApplyRewrittenNoteEncodeError)
...
func coverageApplyRewrittenNoteEncodeError(t *testing.T) {
	root := testRepo(t)
	const filesPerCommit = 251
	for index := 0; index < filesPerCommit; index++ {
		write(t, root, fmt.Sprintf("first-%03d.txt", index), fmt.Sprintf("first-%03d\n", index))
	}
	oldOne := commit(t, root, "old one")
	for index := 0; index < filesPerCommit; index++ {
		write(t, root, fmt.Sprintf("second-%03d.txt", index), fmt.Sprintf("second-%03d\n", index))
	}
	oldTwo := commit(t, root, "old two")
	target := commitWithMessage(t, root, "target")
	repo, err := gitcmd.Discover(root)
	if err != nil {
		t.Fatal(err)
	}
	// One ls-tree per commit replaces one git subprocess per file: a
	// rapid fork/exec storm under the full parallel gate made git fail
	// on macOS with EINVAL while creating loose object temp files.
	blobs := func(commitID, prefix string) map[string]string {
		out := git(t, root, "ls-tree", "-r", "-z", commitID)
		files := make(map[string]string, filesPerCommit)
		for _, entry := range strings.Split(strings.TrimSuffix(out, "\x00"), "\x00") {
			meta, path, ok := strings.Cut(entry, "\t")
			if !ok {
				continue
			}
			if _, wanted := strings.CutPrefix(path, prefix); !wanted {
				continue
			}
		...
		if len(files) != filesPerCommit {
			t.Fatalf("ls-tree found %d %s files, want %d", len(files), prefix, filesPerCommit)
		}
		return files
	}
	note := func(commitID, prefix string) model.Note {
		ids := blobs(commitID, prefix)
		files := make(map[string]model.NoteFile, filesPerCommit)
		for index := 0; index < filesPerCommit; index++ {
			path := fmt.Sprintf("%s-%03d.txt", prefix, index)
		...
		return model.Note{Version: model.NoteVersion, Files: files}
	}
	if err := repo.WriteNote(oldOne, encodeCoverageNote(t, note(oldOne, "first"))); err != nil {
		t.Fatal(err)
	}
	if err := repo.WriteNote(oldTwo, encodeCoverageNote(t, note(oldTwo, "second"))); err != nil {
		t.Fatal(err)
	}
	mapping, err := rewrite.NewMapping([]rewrite.Pair{
	...
	if _, err := applyRewriteMapping(repo, mapping); err == nil || !strings.Contains(err.Error(), "encode rewritten note") {
		t.Fatalf("applyRewriteMapping accepted an oversized merged note: %v", err)
	}
}
