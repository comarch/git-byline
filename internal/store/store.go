...
func (store Store) AppendCheckpoint(record model.Checkpoint) error {
	...
	data = append(data, '\n')
	file, err := os.OpenFile(store.CheckpointPath(), os.O_RDWR|os.O_CREATE|os.O_APPEND, 0o600)
	if err != nil {
		return fmt.Errorf("open checkpoint log: %w", err)
	}
	prefix, err := repairCheckpointTail(file)
	if err != nil {
		file.Close()
		return err
	}
...
func repairCheckpointTail(file *os.File) ([]byte, error) {
