...
- Install CodeRabbit with access limited to this repository and keep its
  repository-local knowledge scopes and external web search disabled.
- Verify Codecov reports the expected project and patch status names before
  making them required.
- Verify CodeRabbit reports its expected review status on same-repository and
  fork pull requests before making it required.
